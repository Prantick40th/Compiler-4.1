...
void parentheses_check()
{
    ...
    else
    {
        while((c=fgetc(fp))!=EOF)
        {
            ...
            {
                if(st.empty()||!bracket_pair(st.top(), c)) //have end but stuck empty
                {
...
void insert_id()
{
    ...
    char str[1000];
    while(fgets(str, sizeof str, fp)!=NULL){ //take the line into str
        char *p=str; // pointer of str
        char word[sizeof str]; // word to check
        int n;
        while(sscanf(p, "%s%n", word, &n)==1){ //word = single string , n = address
            //printf("%s",word);
            //cout<<" "<<&n;
            cout<<endl;
            if(keyword(word))
